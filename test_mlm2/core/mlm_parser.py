...
def process_pck_content(module_code: str,content: str, message_mappings: dict) -> str:
    """
    ...
    for match in reversed(matches):
        ...
        if original_msg in message_mappings:
            ...
            if any(pattern in stmt for pattern in ['Raise_Application_Error', 'Ut.Raise_Err', 'Em.Raise_Error']):
                ...
                replacement = generate_mlm_raise_error(module_code,message_name, params)
            else:
                params = parse_error_params(stmt)
                replacement = generate_mlm_replacement(module_code,message_name, stmt, params)
...
def generate_mlm_replacement(module_code: str,message_name: str, original_stmt: str, params: List[str] = None) -> str:
    ...
    if not params:
        return f"{variable_name} := mlm.Get_Message(i_Module_Code => '{module_code}', i_Message_Name => '{message_name}');"
    ...
    result = f"{variable_name} := mlm.Get_Message(i_Module_Code => '{module_code}', i_Message_Name => '{message_name}', i_Params => {param_array});"
...
def generate_mlm_raise_error(module_code: str,message_name: str, params: List[str] = None) -> str:
    """Generate MLM Raise_Error call with proper parameter handling"""
    if not params:
        return f"mlm.Raise_Error(i_Module_Code => '{module_code}', i_Message_Name => '{message_name}');"
    ...
    result = f"mlm.Raise_Error(i_Module_Code => '{module_code}', i_Message_Name => '{message_name}', i_Params => {param_array});"
