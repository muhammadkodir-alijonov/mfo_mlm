...
import uuid
import asyncio
from fastapi import FastAPI, File, UploadFile, WebSocket, BackgroundTasks, HTTPException, Form
...
uploaded_files_metadata: Dict[str, Dict] = {}
...
async def upload_file(
    ...
    module_code: str = Form(...)  # Add module_code as form parameter
):
    # Validate file extension
    if not file.filename.endswith('.pck'):
        raise HTTPException(status_code=400, detail="Only .pck files are allowed")
    ...
    # Store metadata including module code
    uploaded_files_metadata[file_id] = {
        "module_code": module_code
    }

    logger.info(f"File uploaded: {file.filename} as {file_location} with module {module_code}")
...
async def process_convert_task(file_id: str, file_path: str, original_filename: str):
    try:
        module_code = uploaded_files_metadata[file_id]["module_code"]
        # Update status
        processing_status[file_id]["message"] = "Converting file to MLM..."

        # Simulate progress updates
        for i in range(1, 10):
            processing_status[file_id]["progress"] = i * 5
        ...
        # Actual processing
        output_dir = "outputs"
        output_path = convert_to_mlm(module_code,file_path, output_dir)
...
async def process_script_task(file_id: str, file_path: str, original_filename: str):
    try:
        ...
        # Actual processing
        output_dir = "outputs"
        output_path = make_script_for_mlm(file_path, output_dir,module_code)

        if output_path:
            base_name = os.path.splitext(original_filename)[0]
            output_filename = f"{base_name}_Inserts.sql"
            final_path = f"outputs/{file_id}_{output_filename}"
