...
def extract_error_message(raise_stmt: str) -> Optional[str]:
    ...
    try:
        # CASE WHEN strukturasini tekshirish
        if 'case' in raise_stmt.lower() and 'when' in raise_stmt.lower():
            pattern = r"when\s+'([^']+)'\s+then\s+'([^']+)'"
            matches = re.finditer(pattern, raise_stmt, re.IGNORECASE)
            cases = {}
            for match in matches:
                cases[match.group(1)] = match.group(2)

            # ELSE qismini qayta ishlash
            else_pattern = r"else\s+'([^']+)'"
            else_match = re.search(else_pattern, raise_stmt, re.IGNORECASE)
            if else_match:
                cases['else'] = else_match.group(1)
            return str(cases)

        # Parametrli va oddiy xabarlarni qayta ishlash
